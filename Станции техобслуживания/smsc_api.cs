// WWW3.SMSC.RU API (www3.smsc.ru) версия 3.0 (31.01.2017)
using System;
using System.Web;
using System.Net;
using System.IO;
...
using System.Text;
using Станции_техобслуживания;

public class SMSC
{
	// Константы с параметрами отправки
	const string SMSC_LOGIN = "Hasdik";		    // логин клиента
	const string SMSC_PASSWORD = "9c684b7630e41690e7cb23e9c3a25492";	// пароль или MD5-хеш пароля в нижнем регистре
	...
	const string SMTP_FROM = "api@www3.smsc.ru";		// e-mail адрес отправителя
	const string SMTP_SERVER = "send.www3.smsc.ru";	// адрес smtp сервера
	...
	public void send_sms_mail(string phones, string message, int translit = 0, string time = "", int id = 0, int format = 0, string sender = "")
	{
		...
		mail.To.Add("send@send.www3.smsc.ru");
	...
	private string[] _smsc_send_cmd(string cmd, string arg, string[] files = null)
	{
		...
		url = _url = (SMSC_HTTPS ? "https" : "http") + "://www3.smsc.ru/sys/" + cmd + ".php" + (SMSC_POST ? "" : "?" + arg);
        ...
        {
            if (i++ > 0)
                url = _url.Replace("www3.smsc.ru/", "www" + i.ToString() + ".www3.smsc.ru/");
...
// string[] r = smsc.send_sms("79999999999", "http://www3.smsc.ru\nWWW3.SMSC.RU", 0, "", 0, 0, "", "maxsms=3");
